"""OKX GPT 自动止盈止损 — 中文终端面板。

覆盖：OKX API、模型 API（GPT + Grok）、Telegram、保护参数、
...
# --- 所有密钥（不回显） ---
ALL_SECRET_KEYS = OKX_CREDENTIAL_KEYS + MODEL_CREDENTIAL_KEYS + TELEGRAM_CREDENTIAL_KEYS

# --- OKX 实时查询所需 ---
REQUIRED_OKX_KEYS = OKX_CREDENTIAL_KEYS
...
def _okx_status(values: dict[str, str]) -> str:
    missing = [k for k in REQUIRED_OKX_KEYS if not values.get(k)]
    if not missing:
        return "✅ 已配置"
...
def _circuit_breaker_status(values: dict[str, str]) -> str:
    ...
    try:
        ...
        return f"✅ 正常（失败 {data.get('consecutive_failures', 0)} 次）"
...
def _protection_orders_status(values: dict[str, str]) -> str:
    ...
    try:
        ...
        if not orders:
            return "  无托管单"
        lines = []
        for inst, info in orders.items():
            algo_id = info.get("algoId", "?")
            stop = info.get("stop_loss", "?")
            tp = info.get("take_profit", "-")
            lines.append(f"  {inst}: 止损={stop} 止盈={tp} (algoId={algo_id})")
        return "\n".join(lines)
    except (json.JSONDecodeError, OSError):
        return "  ❌ 读取失败"
...
def _cron_hint() -> str:
    ...
    return "*/30 * * * * cd /root/okx-demo-auto-protection && .venv/bin/python auto_runner.py >> logs/auto-runner.log 2>&1"
...
def render_menu(values: dict[str, str]) -> str:
    okx = _okx_status(values)
    ...
    lines = [
        "",
        "╔══════════════════════════════════════════════════════════╗",
        "║     OKX 模拟盘自动止盈止损 — GPT 风控 + Telegram 推送      ║",
        "╠══════════════════════════════════════════════════════════╣",
        f"║  OKX: {okx}  GPT: {gpt}  Grok: {grok}",
        f"║  TG: {tg}  保护: {prot}  熔断: {cb}",
        "╠══════════════════════════════════════════════════════════╣",
        "║  【配置】                                                ",
        "║  1. OKX API     （模拟盘 key / secret / passphrase）     ",
...
def _action_configure_okx(path: Path) -> None:
    values = load_env(path)
    print()
    print("══ OKX 模拟盘 API ══")
    print("输入空值保留当前配置；密钥不回显。")
    print()
    for key in OKX_CREDENTIAL_KEYS:
    ...
    print()
    # Base URL
    base_key = "OKX_API_BASE"
    default = OPTIONAL_DEFAULTS.get(base_key, "https://www.okx.com")
    current = values.get(base_key, default)
    value = _prompt_text("OKX_API_BASE（接口地址）", current)
...
def _action_full_status(path: Path) -> None:
    ...
    print(f"  OKX_API_BASE: {values.get('OKX_API_BASE', 'https://www.okx.com')}")
    print()
    print("══ GPT 风险模型 ══")
...
def _action_run_auto_cycle(path: Path) -> None:
    values = load_env(path)
    if not values.get("OKX_DEMO_API_KEY"):
        print("\n  ❌ 请先配置 OKX API（菜单 1）")
        return
    print()
...
def _action_toggle_protection(path: Path) -> None:
    ...
    if not current:
        print()
        print("  当前状态: 🔴 关闭")
        print("  开启后将自动在 OKX 模拟盘挂 reduceOnly 条件止盈止损单。")
        print()
...
def run_menu(path: Path = ENV_PATH) -> None:
    ...
    actions: dict[str, Callable[[], None]] = {
        # 配置
        "1": lambda: _action_configure_okx(path),
