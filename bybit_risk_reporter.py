"""Deterministic, credential-free core for Bybit linear perpetual risk reports."""
...
def position_from_snapshot(raw: dict[str, Any]) -> Position:
    ...
    if not instrument or (not instrument.endswith("-SWAP") and not instrument.endswith("USDT")):
        raise ValidationError("仅支持线性永续合约持仓")
...
def build_report(snapshot: dict[str, Any], *, stop_pct: float | None = None) -> str:
    ...
    source = f"最近成功缓存（{cached_at}，可能过期）" if cached_at else "Bybit Demo实时查询"
    ...
    risks = [
        ...
        (f"3. 当前标记价距离强平价约{liq_distance:.2f}%，强平价会随账户状态变化。"
         if liq_distance is not None else "3. Bybit未返回强平价，无法计算强平距离。"),
    ...
    return "\n".join([
        ...
        *fib_lines,
        ...
        "**持仓风险：**", *risks,
        "**说明：仅根据Bybit Demo实际持仓币种分析，不显示固定BTC价格；不执行交易。**",
    ])
